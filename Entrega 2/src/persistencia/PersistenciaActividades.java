...
public class PersistenciaActividades {
	private String[] titulos = {"ID", "descripcion", "objetivo", "tipo", "contenido", "nivelDificultad", "duracionEsperada", "preRequisitos"};
	...
	public void cargarArchivo(String ruta, List<Actividad> actividades) throws JSONException, IOException {
		...
		for (int i = 0; i < jActividades.length(); i++) {
			...
			if (jActividad.getString(titulos[3]).equals("RE")) {
				//actividad = new RecursoEducativo();
	...
	public void guardarArchivo(String ruta, List<Actividad> actividades) {
		...
		for (Actividad actividad: actividades) {
			...
			for (int i = 0; i < titulos.length-1; i++) {
				if (i == 4) {
					jActividad.put(titulos[i], guardarContenido(actividadArray[i-1], actividadArray[i]));
				}else {
			...
			if (!actividadArray[7].equals("NA")) {
				String[] actividadesPrev = actividadArray[7].split("%");
			...
			jActividad.put(titulos[7], jActividades);
